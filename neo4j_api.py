...
def load_data(file_path, size, songs_of_interest=False, remove_artist=False):

    # Load CSV file into a DataFrame
    spotify_df = pd.read_csv(file_path) 

    # Drop track_genre column
    spotify_df.drop('track_genre', axis=1, inplace=True)

    # Remove duplicates
    spotify_df.drop_duplicates(inplace=True)

    # Remove instances where artists column matches remove_artist value
    if remove_artist:
        df_removed_artist = spotify_df[spotify_df['artists'] != remove_artist] 
        # Takes a sample of the spotify_df
        df = df_removed_artist.sample(size)
    
    # Takes a sample of the spotify_df
    if remove_artist == False:
        df = spotify_df.sample(size)

    # Adds songs_of_interest to the randomly sampled spotify df 
